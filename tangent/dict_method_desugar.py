...
def _is_dict_literal_with_const_keys(node):
    return (
        isinstance(node, gast.Dict)
        and node.keys
        and all(isinstance(k, gast.Constant) for k in node.keys)
    )
...
class _StaticDictCollector(gast.NodeVisitor):
    ...
    def static_keys(self):
        return {name: keys for name, keys in self.keys.items() if self.assign_count.get(name) == 1}
...
class DictMethodDesugarer(gast.NodeTransformer):
    ...
    def _sum_of_values(self, node):
        ...
        inner = node.args[0]
        if not (
            isinstance(inner, gast.Call)
            and isinstance(inner.func, gast.Attribute)
        ...
        if _is_dict_literal_with_const_keys(receiver):
            ...
            value_exprs = [
                gast.Subscript(
                    value=copy.deepcopy(receiver), slice=copy.deepcopy(k), ctx=gast.Load()
                )
                for k in keys
            ]
        else:
    ...
    def visit_Call(self, node):
        ...
        if (
            ...
            and not any(isinstance(a, gast.Starred) for a in node.args)
            ...
            subscript = gast.Subscript(
                value=copy.deepcopy(receiver), slice=copy.deepcopy(key), ctx=gast.Load()
            )
            ...
            ifexp = gast.IfExp(
                test=gast.Compare(
                    left=copy.deepcopy(key), ops=[gast.In()], comparators=[copy.deepcopy(receiver)]
                ),
                body=subscript,
                orelse=default,
            )
