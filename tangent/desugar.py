...
def _references_active(node, active):
    ...
    for n in gast.walk(node):
        if isinstance(n, gast.Name) and isinstance(n.ctx, gast.Load) and n.id in active:
            return True
    return False
...
class ExplicitLoopIndexes(transformers.TreeTransformer):
    ...
    def _reject_unsupported_iter(self, node):
        ...
        if (
            ...
            and isinstance(it.func, gast.Attribute)
            and it.func.attr in ('values', 'keys', 'items')
            and _references_active(it.func.value, active)
        ):
