...
class ForwardAD(transformers.TreeTransformer):
        ...
        if self.check_dims:
            ...
            def shape_match_template(primal, tangent_):
                if not tangent.shapes_match(primal, tangent_):
                    raise ValueError(
                        ...
                        '(%s)' % (numpy.shape(primal), numpy.shape(tangent_))
                    )
            ...
            for iwrt, tangent_var in zip(self.wrt, grad_args):
                ...
                shape_check = template.replace(
                    shape_match_template, primal=primal, tangent_=tangent_var
                )[0]
    ...
    def visit_Assign(self, node):
        ...
        for i in range(len(new_node)):
            new_node[i] = comments.add_comment(
                new_node[i], 'Primal and tangent of: %s' % quoting.unquote(node)
            )
        return new_node
    ...
    def visit_Call(self, node):
        ...
        if func == tracing.Traceable:
            raise NotImplementedError(
                'Tracing of %s is not enabled in forward mode' % quoting.unquote(node)
            )
        ...
        if func not in tangents.tangents:
            ...
            for i in active_args:
                ...
                arg_grad_node = create.create_grad(orig_args.args[i], self.namer, tangent=True)
                grad_node.ctx = gast.Load()
                tangent_keywords.append(gast.keyword(arg=arg_grad_node.id, value=grad_node))
        ...
        if flags & inspect.CO_VARARGS:
            ...
            arg_replacements[vararg_name] = gast.Tuple(elts=to_pack, ctx=gast.Load())
        tangent_node = template.replace(
            template_, replace_grad=template.Replace.TANGENT, namer=self.namer, **arg_replacements
        )
    ...
    def visit_Return(self, node):
        ...
        if isinstance(retval, (gast.Name, gast.Subscript)):
            retval = gast.Tuple(
                elts=[create.create_grad(retval, self.namer, tangent=True)], ctx=gast.Load()
            )
        elif isinstance(retval, gast.Tuple):
            retval.elts = [create.create_grad(elt, self.namer, tangent=True) for elt in retval.elts]
        else:
            raise ValueError
