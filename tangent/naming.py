...
def primal_name(func, wrt):
    ...
    if not isinstance(func, types.FunctionType):
        ...
        if hasattr(func, '__wrapped__'):
            ...
            while hasattr(unwrapped, '__wrapped__') and not isinstance(
...
class Namer(object):
    ...
    def name_Call(self, node):
        if len(node.args) <= 2:
            return self._name(node.func) + '_' + '_'.join(self._name(arg) for arg in node.args)
        else:
    ...
    def name_BinOp(self, node):
        return '{left}_{op}_{right}'.format(
            left=self._name(node.left),
            right=self._name(node.right),
            op=self.BINOP_NAMES[type(node.op)],
        )

    UNARYOP_NAMES = {gast.UAdd: 'plus', gast.USub: 'minus', gast.Not: 'not'}

    def name_UnaryOp(self, node):
        return '{op}_{operand}'.format(
            op=self.UNARYOP_NAMES[type(node.op)], operand=self._name(node.operand)
        )
    ...
    BOOLOP_NAMES = {gast.And: 'and', gast.Or: 'or'}
    ...
    def name_IfExp(self, node):
        ...
        return '{test}_if_{body}_else_{orelse}'.format(
            test=self._name(node.test), body=self._name(node.body), orelse=self._name(node.orelse)
        )
