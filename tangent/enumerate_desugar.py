...
def _range_len(iterable):
    return gast.Call(
        ...
        args=[
            gast.Call(func=_name('len', gast.Load()), args=[copy.deepcopy(iterable)], keywords=[])
        ],
        keywords=[],
    )
...
class EnumerateDesugarer(gast.NodeTransformer):
    ...
    def visit_For(self, node):
        ...
        if not (isinstance(idx_target, gast.Name) and isinstance(val_target, gast.Name)):
            return node
        ...
        if start is None:
            ...
            idx_assign = _assign(
                ...
                gast.BinOp(left=copy.deepcopy(start), op=gast.Add(), right=_name(pos, gast.Load())),
            )
            val_assign = _assign(
                _name(val_target.id, gast.Store()),
                gast.Subscript(
                    value=copy.deepcopy(iter_ref), slice=_name(pos, gast.Load()), ctx=gast.Load()
                ),
            )
