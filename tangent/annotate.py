        ...
        if func is None and isinstance(node.func, gast.Attribute):
            ...
            if resolver is not None:
                ...
                node.func = gast.Attribute(
                    value=resolver.base_node(self.namespace), attr=method_name, ctx=gast.Load()
                )
...
class FindStackOps(gast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        if fn_handle and fn_handle in [utils.pop, utils.push, utils.push_stack, utils.pop_stack]:
            ...
            assert fn_handle not in self.push_pop_pairs, (
                'Conflicting op_ids. Already have fn %s with op_id %s'
            ) % (quoting.unquote(node.func), op_id)
...
class AnnotateStacks(gast.NodeVisitor):
    ...
    def visit_Assign(self, node):
        ...
        if fn_handle and fn_handle in [utils.pop, utils.pop_stack]:
            ...
            if (
                self.strict
                and set(keys) != set((utils.push, utils.pop))
...
class Unused(gast.NodeVisitor):
    ...
    def visit(self, node):
        ...
        if (
            isinstance(node, gast.Name)
            and isinstance(node.ctx, gast.Load)
            and self.reaching_definitions is not None
        ):
            self.used.update(def_ for def_ in self.reaching_definitions if def_[0] == node.id)
