...
def copy_node(node):
    ...
    setattr(new_node, anno.ANNOTATION_FIELD, getattr(node, anno.ANNOTATION_FIELD, {}).copy())
    return new_node
...
def is_insert_grad_of_statement(node):
    ...
    """
    tangent_calls = [
        anno.getanno(item.context_expr, 'func', None) is utils.insert_grad_of for item in node.items
    ]
