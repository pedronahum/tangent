...
_BACKEND_HELPERS = {
    ...
    'torch': {'cat': 'torch_cat_seq', 'concat': 'torch_cat_seq',
              'concatenate': 'torch_cat_seq', 'stack': 'torch_stack_seq'},
...
def _module_chain_matches(value, chain):
  ...
  return (isinstance(value, gast.Attribute) and
          isinstance(value.value, gast.Name) and
          value.value.id == chain[0] and
          value.attr == chain[1])
