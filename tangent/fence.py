...
class _NestedFunctionFinder(ast.NodeVisitor):
    ...
    def _visit_def(self, node):
        if self._function_depth >= 1:
            ...
            if suggestion:
                message = error_suggestions.format_error_with_suggestion(feature, message)
...
class LanguageFence(ast.NodeVisitor):
    ...
    def visit_NamedExpr(self, node):
        ...
        self._reject(node, 'Assignment expressions (the walrus operator ":=") are not supported')
    ...
    def visit_AugAssign(self, node):
        ...
        if isinstance(node.target, ast.Attribute):
            self._reject(node, 'Augmented assignment to an attribute is not supported')
        else:
            self._allow_and_continue(node)
    ...
    def visit_arguments(self, node):
        ...
        if node.vararg is not None:
            self._reject(node, 'Variadic positional arguments (*args) are not supported')
        if node.kwarg is not None:
            self._reject(node, 'Variadic keyword arguments (**kwargs) are not supported')
        self._allow_and_continue(node)
