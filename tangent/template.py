...
class ReplaceGradTransformer(transformers.TreeTransformer):
    ...
    def visit_Subscript(self, node):
        ...
        if isinstance(node.value, gast.Name) and node.value.id == 'd':
            ...
            elif self.replace_grad in (Replace.FULL, Replace.TANGENT) or isinstance(
                node.ctx, gast.Load
            ):
                new_node = create.create_grad(slice_value, self.namer, self.tangent)
            elif isinstance(node.ctx, gast.Store):
                new_node = create.create_temp_grad(slice_value, self.namer, self.tangent)
            else:
                raise ValueError
            new_node.ctx = node.ctx
...
def replace(template, replace_grad=Replace.PARTIAL, namer=None, **replacements):
    ...
    if replace_grad is not Replace.NONE:
        rgt = ReplaceGradTransformer(
            replace_grad=replace_grad, namer=namer, tangent=replace_grad is Replace.TANGENT
        )
        rgt.visit(tree)
