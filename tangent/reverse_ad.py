...
class ReverseAD(object):
    ...
    def __init__(
        self, wrt, preserve_result, check_dims, checkpoint_config=None, reconcile_seed=False
    ):
        self.required = []
        self.wrt = wrt
    ...
    def is_active(self, node):
        ...
        if (
            ...
            and anno.getanno(node.value, 'func', False) == utils.pop
        ):
            return True
        for succ in gast.walk(node.value):
            if (
                isinstance(succ, gast.Name)
                and isinstance(succ.ctx, gast.Load)
                and succ.id in self.active_variables
            ):
                return True
        return False
    ...
    def visit_FunctionDef(self, node):
        ...
        if adjoint_body:
            adjoint_body[0] = comments.add_comment(adjoint_body[0], 'Beginning of backward pass')

        # Before updating the primal arguments, extract the arguments we want
        # to differentiate with respect to
        dx = gast.Tuple(
            [create.create_grad(node.args.args[i], self.namer) for i in self.wrt], ctx=gast.Load()
        )
        ...
        if self.preserve_result:
            ...
            assign_stored_result = template.replace(
                'result=orig_result', result=stored_result_node, orig_result=return_node.value
            )
        ...
        if isinstance(y, gast.Tuple):
            y = y.elts[0]
        dy = gast.Name(id=self.namer.grad(y.id), ctx=gast.Param(), annotation=None)

        if self.check_dims:

            def shape_match_template(primal, adjoint):
                assert tangent.shapes_match(primal, adjoint), (
        ...
        if self.reconcile_seed and output_arity is None and isinstance(y, gast.Name):
            ...
            # value.
            recon = quoting.quote('%s = tangent.match_seed(%s, %s)' % (dy.id, y.id, dy.id))
        ...
        (adjoint,) = template.replace(
            adjoint_template, namer=self.namer, adjoint_body=adjoint_body, return_dx=return_dx
        )
    ...
    def visit_With(self, node):
        ...
        if ast_.is_insert_grad_of_statement(node):
            ...
            for item in node.items:
                if not isinstance(item.context_expr.args[0], gast.Name) or not isinstance(
                    item.optional_vars, gast.Name
                ):
                    raise ValueError
    ...
    def visit_IfExp(self, node):
        ...
        push_cond = template.replace(
            'push(_stack, cond, op_id)', push=push, _stack=self.stack, cond=cond_var, op_id=op_id
        )
        ...
        assign = gast.Assign(targets=[ast_.copy_node(self.orig_target)], value=ifexp_expr)
        ...
        pop_cond = template.replace(
            'cond = pop(_stack, op_id)', cond=cond_var, pop=pop, _stack=self.stack, op_id=op_id
        )
        ...
        body_adj_list = (
            body_adjoint
            if isinstance(body_adjoint, list)
            else ([body_adjoint] if body_adjoint else [])
        )
        orelse_adj_list = (
            orelse_adjoint
            if isinstance(orelse_adjoint, list)
            else ([orelse_adjoint] if orelse_adjoint else [])
        )
    ...
    def visit_AugAssign(self, node):
        raise ValueError(
            'AugAssign not supported for gradients. Should have been removed by ANF. This is a bug.'
        )
    ...
    def visit_Assign(self, node):
        ...
        if (
            isinstance(node.value, gast.Call)
            and anno.hasanno(node.value, 'func')
            and anno.getanno(node.value, 'func') in (utils.Stack, utils.pop_stack)
        ):
        ...
        store = template.replace(
            'push(_stack, y, op_id)', push=push, y=self.orig_target, _stack=self.stack, op_id=op_id
        )
        create_substack = template.replace('substack = tangent.Stack()', substack=self.substack)
        ...
        self.target = create.create_temp(self.orig_target, self.namer)
        create_tmp = template.replace('tmp = y', tmp=self.target, y=self.orig_target)
        ...
        try:
            ...
            raise ValueError('Failed to process assignment to: %s. Error: %s' % (context, e))
        ...
        if isinstance(fx, gast.Assign):
            assign = [fx]
        elif isinstance(fx, list) and any([isinstance(ifx, gast.Assign) for ifx in fx]):
            assign = fx
        else:
            assign = template.replace('y = fx', y=ast_.copy_node(self.orig_target), fx=fx)
            assign = [assign]
        ...
        # the partials
        adjoint = [create_tmp, restore_substack, restore] + adjoint_rhs + [reset] + accumulations
        ...
        if isinstance(slice_value, gast.Name):
            ...
            push_index = template.replace(
                'push(_stack, i, op_id)', push=push, i=i, _stack=self.stack, op_id=op_id
            )
            pop_index = template.replace(
                'i = pop(_stack, op_id)', pop=pop, i=i, _stack=self.stack, op_id=op_id
            )
    ...
    def visit_Name(self, node):
        ...
        is_subscript_target = anno.hasanno(target_node, 'temp_var') and isinstance(
            anno.getanno(target_node, 'temp_var'), gast.Subscript
        )
        if is_subscript_target:
            adjoint = template.replace(
                'd[x] = tangent.unbroadcast(d[y], x)', namer=self.namer, x=node, y=target_node
            )
        else:
            adjoint = template.replace(
                'd[x] = tangent.copy(d[y])', namer=self.namer, x=node, y=target_node
            )
        return node, adjoint
    ...
    def visit_Dict(self, node):
        ...
        for key_node, value_node in zip(node.keys, node.values):
            ...
            grad_subscript = gast.Subscript(value=grad_target, slice=key_node, ctx=gast.Load())

            # Create the assignment: d_value = d_result[key]
            adjoint.append(
                template.replace('d[x] = rhs', namer=self.namer, x=value_node, rhs=grad_subscript)
            )
    ...
    def visit_container(self, node):
        ...
        for i, elt in enumerate(node.elts):
            ...
            grad_target_subscript = gast.Subscript(
                value=grad_target, slice=index_node, ctx=gast.Load()
            )

            # Now create the assignment: d[x] = d[t][i]
            adjoint.append(
                template.replace('d[x] = rhs', namer=self.namer, x=elt, rhs=grad_target_subscript)
            )
        return node, adjoint
    ...
    def visit_Try(self, node):
        ...
        for handler in node.handlers:
            ...
            new_handler = gast.ExceptHandler(
                type=handler.type, name=handler.name, body=handler_body
            )
            handlers.append(new_handler)

            # Create adjoint handler
            adjoint_handler = gast.ExceptHandler(
                type=handler.type, name=handler.name, body=handler_adjoint
            )
        ...
        primal = gast.Try(body=body, handlers=handlers, orelse=orelse, finalbody=finalbody)
    ...
    def visit_BinOp(self, node):
        ...
        adjoint = template.replace(
            adjoint_template, namer=self.namer, x=node.left, y=node.right, z=self.target
        )
        return node, adjoint
    ...
    def visit_UnaryOp(self, node):
        ...
        adjoint_template = grads.adjoints[op]
        adjoint = template.replace(
            adjoint_template, namer=self.namer, x=node.operand, y=self.target
        )
        return node, adjoint
    ...
    def primal_and_adjoint_for_tracing(self, node):
        ...
        dto_pack = gast.Tuple(
            elts=[create.create_temp_grad(arg, self.namer) for arg in to_pack], ctx=gast.Store()
        )

        adjoint = template.replace(
            adjoint_template, namer=self.namer, result=target, vjp=vjp, dargs=dto_pack
        )
    ...
    def visit_Call(self, node):
            ...
            for j, i in enumerate(active_args):
                adjoint.append(
                    template.replace(
                        ...
                        namer=self.namer,
                        x=node.args[i].id,
                        i=gast.Constant(value=j, kind=None),
                    )
                )
        ...
        replace_grad = (
            template.Replace.FULL if flags & inspect.CO_VARARGS else template.Replace.PARTIAL
        )
        adjoint = template.replace(
            template_, namer=self.namer, replace_grad=replace_grad, **arg_replacements
        )
        unpacking = []
        if flags & inspect.CO_VARARGS:
            ...
            dto_pack = [create.create_temp_grad(arg, self.namer) for arg in to_pack]
    ...
    def visit_Expr(self, node):
        ...
        adjoint = []
        if isinstance(node.value, gast.Call) and anno.getanno(node.value, 'func') == utils.push:
            orig_src = quoting.unquote(node)
...
def reverse_ad(
    node, wrt, preserve_result, check_dims, checkpoint_config=None, reconcile_seed=False
):
...
def store_state(node, reaching, defined, stack):
    ...
    for id_, def_ in zip(reaching, original_defs):
        ...
        store.append(
            template.replace(
                'push(_stack, val, op_id)', push=push, val=id_, _stack=stack, op_id=op_id
            )
        )
        load.append(
            template.replace(
                'val = pop(_stack, op_id)', pop=pop, val=id_, _stack=stack, op_id=op_id
            )
        )
...
def joint(node):
    ...
    if anno.hasanno(node.body[0], 'output_arity'):
        anno.setanno(
            func.body[0], 'output_arity', anno.getanno(node.body[0], 'output_arity'), safe=False
        )
