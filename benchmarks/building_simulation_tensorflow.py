"""
Building Simulation Benchmark - TensorFlow Implementation

This implements the same building thermal simulation as:
https://github.com/PassiveLogic/differentiable-swift-examples/

Benchmarks TensorFlow's automatic differentiation on the building simulation.
"""
...
# Type definitions (represented as TensorFlow tensors)
...
def run_benchmark():
    ...
    print("\n" + "=" * 80)
    print("BUILDING SIMULATION BENCHMARK - TENSORFLOW")
    print("=" * 80)
    ...
    for i in range(TRIALS + WARMUP):
        # Forward pass
        forward_time, forward_output = measure(full_pipe, SIM_PARAMS)

        # Backward pass
        sim_params_var = tf.Variable(SIM_PARAMS)
        gradient_time, gradient = measure(get_gradient, sim_params_var)
