...
settings.register_profile('deterministic', derandomize=True, max_examples=25, deadline=None)
settings.register_profile('fuzz', derandomize=False, max_examples=200, deadline=None)
settings.load_profile(os.environ.get('TANGENT_HYPOTHESIS_PROFILE', 'deterministic'))
...
scalar_inputs = st.floats(min_value=-2.0, max_value=2.0, allow_nan=False, allow_infinity=False)
array_inputs = hnp.arrays(
    dtype=np.float64, shape=st.integers(min_value=3, max_value=5), elements=scalar_inputs
)
...
def _forward_diff(fn):
    functions.np = np
    return tangent.autodiff(fn, mode='forward', wrt=(0,), preserve_result=False, optimized=True)
...
def _check_matches_fd(dfn, fn, *args):
    ...
    assert np.allclose(got, fd, rtol=1e-3, atol=1e-5), (
        '%s: tangent gradient %r != finite differences %r at %r' % (fn.__name__, got, fd, args)
    )
...
@pytest.mark.parametrize('fn,boundaries', SCALAR_CASES, ids=[f.__name__ for f, _ in SCALAR_CASES])
...
@pytest.mark.parametrize('fn', ARRAY_CASES, ids=[f.__name__ for f in ARRAY_CASES])
...
def test_saxpy_grad_matches_finite_differences(a0, b0, c0):
    ...
    _check_matches_fd(_reverse_grad(functions.saxpy), functions.saxpy, a0, b0, c0)
...
def test_multivar_if_grad_matches_finite_differences(a0, b0):
    ...
    _check_matches_fd(_reverse_grad(functions.multivar_if), functions.multivar_if, a0, b0)


@pytest.mark.parametrize('fn', SMOOTH_SCALAR_FUNCS, ids=[f.__name__ for f in SMOOTH_SCALAR_FUNCS])
@given(a0=scalar_inputs)
def test_forward_mode_matches_finite_differences(fn, a0):
    ...
    assert np.allclose(got, fd, rtol=1e-3, atol=1e-5), (
        '%s: forward-mode derivative %r != finite differences %r at %r' % (fn.__name__, got, fd, a0)
    )
...
def test_grad_of_grad_matches_finite_differences_of_grad(fn, a0):
    ...
    got = _gradgrad(fn)(a0)
    assert np.allclose(got, fd, rtol=1e-3, atol=1e-5), (
        '%s: grad-of-grad %r != finite differences of grad %r at %r' % (fn.__name__, got, fd, a0)
    )
