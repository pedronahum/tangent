...
def register_parametrizations(metafunc, short):
    ...
    def _parametrize_pair(name1, name2, values):
        ...
        """
        both = name1 in metafunc.fixturenames and name2 in metafunc.fixturenames
        ...
        if not tf:
            if both:
                metafunc.parametrize(
                    ...
                    [
                        pytest.param(
                            None, None, marks=pytest.mark.skip(reason='tensorflow not present')
                        )
                    ],
                )
            else:
        ...
        tensors_b = [tf.constant(np.random.randn(*v.shape), dtype=tf.float32) for v in values]
        if both:
    ...
    if 't1' in metafunc.fixturenames or 't2' in metafunc.fixturenames:
        _parametrize_pair('t1', 't2', [np.random.randn(i) for i in ((3,) if short else (3, 5, 10))])

    if 'mat1' in metafunc.fixturenames or 'mat2' in metafunc.fixturenames:
        _parametrize_pair(
            'mat1',
            'mat2',
            [np.random.randn(*i) for i in (((3, 3),) if short else ((1, 1), (3, 3), (5, 5)))],
        )
    ...
    if 'timage1' in metafunc.fixturenames or 'timage2' in metafunc.fixturenames:
        _parametrize_pair('timage1', 'timage2', [np.random.randn(*i) for i in image_shapes])
...
def test_forward_tensor(func, wrt, *args):
    ...
    utils.assert_result_matches_reference(
        tangent_func, reference_func, backup_reference_func, tolerance=1e-4
    )
