...
def check_forward(f, args, wrt=(0,), tol=1e-4):
    ...
    for i in wrt:
        df = tangent.autodiff(f, mode='forward', wrt=(i,), preserve_result=False)
        got = df(*(args + (1.0,)))
...
def add_broadcast(a, b):
    return np.sum(np.add(a, b) * np.array([[0.5, 1.5, -0.5], [2.0, -1.0, 0.25]]))
...
def subtract_broadcast(a, b):
    return np.sum(np.subtract(a, b) * np.array([[0.5, 1.5, -0.5], [2.0, -1.0, 0.25]]))
...
def divide_broadcast(a, b):
    return np.sum(np.divide(a, b) * np.array([[0.5, 1.5, -0.5], [2.0, -1.0, 0.25]]))
...
def multiply_broadcast(a, b):
    return np.sum(np.multiply(a, b) * np.array([[0.5, 1.5, -0.5], [2.0, -1.0, 0.25]]))
...
def fmax_broadcast(a, b):
    return np.sum(np.fmax(a, b) * np.array([[0.5, 1.5, -0.5], [2.0, -1.0, 0.25]]))
...
def test_fmax():
    ...
    check_both(fmax_f, (np.array([1.0, 3.0, 2.0]), np.array([2.0, 1.0, 5.0])), wrt=(0, 1))


def test_fmin():
    check_both(fmin_f, (np.array([1.0, 3.0, 2.0]), np.array([2.0, 1.0, 5.0])), wrt=(0, 1))


def test_fmax_broadcast():
    check_reverse(fmax_broadcast, (M23, np.array([0.5, 4.0, -1.0])), wrt=(0, 1))


def test_maximum_minimum_forward():
    # np.maximum/np.minimum adjoints already existed; the tangents are new.
    check_forward(
        fmax_like_maximum, (np.array([1.0, 3.0, 2.0]), np.array([2.0, 1.0, 5.0])), wrt=(0, 1)
    )
    check_forward(
        fmin_like_minimum, (np.array([1.0, 3.0, 2.0]), np.array([2.0, 1.0, 5.0])), wrt=(0, 1)
    )
...
def test_repeat_array_repeats():
    check_reverse(repeat_array_repeats, (V1.reshape(3, 1), np.linspace(-1.0, 1.0, 6).reshape(6, 1)))
...
def test_solve_vector():
    check_both(solve_vector, (A_GEN, np.array([1.0, 2.0]), np.array([1.0, -2.0])), wrt=(0, 1))


def test_solve_vector_spd():
    check_both(solve_vector, (A_SPD, np.array([1.0, 2.0]), np.array([1.0, -2.0])), wrt=(0, 1))
