...
def test_grad_unary_reduction(func, motion, optimized, preserve_result, timage, boolean):
    """Test gradients of reduction functions."""
    tfe_utils.test_rev_tensor(func, motion, optimized, preserve_result, (0,), timage, boolean)
...
def test_grad_binary_tensor(func, motion, optimized, preserve_result, t1, t2):
    ...
    tfe_utils.test_rev_tensor(func, motion, optimized, preserve_result, (0, 1), t1, t2)


def test_grad_matmul(func, motion, optimized, preserve_result, mat1, mat2, boolean1, boolean2):
    """Test gradients of functions with binary matrix inputs."""
    tfe_utils.test_rev_tensor(
        func, motion, optimized, preserve_result, (0, 1), mat1, mat2, boolean1, boolean2
    )


def test_grad_matmul_higherdim(
    func, motion, optimized, preserve_result, timage1, timage2, boolean1, boolean2
):
    """Test gradients of functions with binary matrix inputs."""
    ...
    if inner1 != inner2:
        pytest.skip(
            'matmul operands incompatible: %s x %s with transpose flags '
            '(%s, %s)' % (s1, s2, boolean1, boolean2)
        )
    tfe_utils.test_rev_tensor(
        func, motion, optimized, preserve_result, (0, 1), timage1, timage2, boolean1, boolean2
    )


def test_grad_tensor_broadcast(func, motion, optimized, preserve_result, s, t):
    """Test gradients of functions with binary tensor inputs."""
    tfe_utils.test_rev_tensor(func, motion, optimized, preserve_result, (0, 1), s, t)


def test_grad_image(func, motion, optimized, preserve_result, timage, tkernel, conv2dstrides):
    """Test gradients of image functions."""
    # TODO: Upgrade utils.py to allow simultaneous testing of uneven args.
    tfe_utils.test_rev_tensor(
        func, motion, optimized, preserve_result, (0,), timage, tkernel, conv2dstrides
    )
    tfe_utils.test_rev_tensor(
        func, motion, optimized, preserve_result, (1,), timage, tkernel, conv2dstrides
    )


def test_grad_image_pooling(
    func, motion, optimized, preserve_result, timage, pool2dsizes, conv2dstrides
):
    tfe_utils.test_rev_tensor(
        func, motion, optimized, preserve_result, (0,), timage, pool2dsizes, conv2dstrides
    )
