...
@pytest.mark.parametrize('fn,expected_dd', [
...
def test_second_derivative_through_tape_optimized(fn, expected_dd, val):
  ...
  got = ddf(val)
  assert np.allclose(got, expected_dd(val)), (
      'ddf(%s) = %s, expected %s' % (val, got, expected_dd(val)))
...
def test_second_derivative_through_branch_optimized():
  ddf = tangent.grad(tangent.grad(branch_grad, optimized=True),
                     optimized=True)
